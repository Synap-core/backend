...
async function runE2ETest() {
  ...
  try {
    ...
    const userMessage = await conversationService.appendMessage({
      ...
      role: "user",
      content:
        "Capture cette idée: planifier une démo LangGraph avec l’équipe produit.",
      userId: testUserId,
    });
    ...
    const agentMetadata = AgentStateSchema.parse({
      ...
      plan:
        agentState.plan?.actions.map((action) => ({
          ...
          reasoning:
            ...
            "Plan fourni sans justification.",
        })) ?? [],
      executionSummaries:
      ...
      suggestedActions: agentState.plan?.actions.map((action) => ({
        type: action.tool,
        description:
          action.justification ?? "Action proposée par le planificateur.",
        params: action.params,
      })),
    });
