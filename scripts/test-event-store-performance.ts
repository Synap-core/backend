 ...
 */
function generateEvents(count: number, userId: string): ReturnType<typeof createSynapEvent>[] {
 ...
 */
async function testSerialInsertion(events: ReturnType<typeof createSynapEvent>[]): Promise<{
  duration: number;
 ...
 */
async function testBatchInsertion(events: ReturnType<typeof createSynapEvent>[]): Promise<{
  duration: number;
  eventsPerSecond: number;
}> {
...
async function main() {
  ...
  try {
    ...
    console.log(`✅ Serial: ${serialResult.duration}ms (${serialResult.eventsPerSecond.toFixed(0)} events/sec)`);
    
    // Test 2: Batch insertion (full set)
    const batchResult = await testBatchInsertion(events);
    console.log(`✅ Batch: ${batchResult.duration}ms (${batchResult.eventsPerSecond.toFixed(0)} events/sec)`);
    
    // Test 3: Time-range query
    const queryResult = await testTimeRangeQuery(userId);
    console.log(`✅ Query: ${queryResult.duration}ms (found ${queryResult.eventCount} events)`);
    
    // Summary
    console.log('\n📊 Performance Summary');
    console.log('════════════════════════════════════════════════════════════');
    console.log(`Serial Insertion:     ${serialResult.eventsPerSecond.toFixed(0)} events/sec`);
    console.log(`Batch Insertion:      ${batchResult.eventsPerSecond.toFixed(0)} events/sec`);
    console.log(`Time-Range Query:     ${queryResult.duration}ms (${queryResult.eventCount} events)`);
    console.log('════════════════════════════════════════════════════════════');
