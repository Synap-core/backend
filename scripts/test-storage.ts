...
async function testStorage() {
  ...
  try {
    ...
    if (builtPath === expectedPath) {
      ...
      console.error(`❌ Path building failed: Expected "${expectedPath}", got "${builtPath}"`);
      process.exit(1);
    }
  } catch (error) {
  ...
  try {
    ...
    console.warn('⚠️  Signed URL test skipped (may not be supported):', error instanceof Error ? error.message : 'Unknown error');
    console.log('');
  }
