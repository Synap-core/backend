...
async function main() {
  ...
  try {
    ...
    logger?.warn({ err: error }, 'Database module could not be loaded - database tests will be skipped');
  ...
  try {
    ...
    logger?.warn({ err: error }, 'Storage module could not be loaded - storage tests will be skipped');
    storage = null;
  }

  logger.info('🧪 Starting System Tests...');
  logger.info({ config: { database: config.database.dialect, storage: config.storage.provider } }, 'Configuration');
  ...
  if (createDatabaseClient) {
    await runTest('Database Connection', async () => {
      ...
      if (config.database.dialect === 'postgres') {
        ...
        const result = await (db as any).select().from({ sql: '1 as test' } as any);
        if (!result) {
        ...
        try {
          ...
          const result = await (db as any).all({ sql: 'SELECT 1 as test' } as any);
          if (!result) {
            throw new Error('Database query returned no result');
          }
        } catch {
    ...
    results.push({ name: 'Database Connection', passed: false, error: 'Database module not loaded (ESM/CJS compatibility)', duration: 0 });
  }
  ...
  if (storage) {
    await runTest('Storage Provider', async () => {
      ...
      try {
        const testPath = await (typeof buildPathMethod === 'function' && buildPathMethod.constructor.name === 'AsyncFunction'
          ? buildPathMethod('test-user', 'note', 'test-id', 'md')
          : Promise.resolve(buildPathMethod('test-user', 'note', 'test-id', 'md')));
        if (!testPath || (!testPath.includes('test-user') && !testPath.includes('note'))) {
        ...
        if (error instanceof Error && error.message.includes('requires') && error.message.includes('R2_')) {
          throw new Error('Storage provider initialized but R2 credentials missing (expected in local dev)');
        }
        throw error;
      }
    });
    ...
    results.push({ name: 'Storage Provider', passed: false, error: 'Storage module not loaded (ESM/CJS compatibility)', duration: 0 });
  }
  ...
  results.forEach((result) => {
    ...
    console.log(`${icon} ${status.padEnd(6)} ${result.name.padEnd(40)} (${result.duration}ms)`);
  ...
  console.log('═'.repeat(60));
  console.log(`Total: ${results.length} tests | Passed: ${passed} | Failed: ${failed} | Duration: ${totalDuration}ms`);
