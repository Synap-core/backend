...
import { mkdtempSync, rmSync, existsSync, mkdirSync, readdirSync, statSync, readFileSync } from 'node:fs';
...
const IGNORE_DIRECTORIES = new Set(['node_modules', '.git', 'dist', 'build', '.turbo', '.pnpm-store']);
...
function walk(current: string, base: string, files: Set<string>, prefix: string): void {
...
function resolveGeneratedProjectRoot(baseDir: string): string {
  ...
  throw new Error(`Unable to determine generated project root within ${baseDir}.`);
}
...
async function main(): Promise<void> {
  ...
  if (cliExecutable && cliExecutable.length > 0) {
    runCommand(cliExecutable, ['new', '--genome', GENOME_PATH, 'synap-generated'], { shell: true, cwd: OUTPUT_DIR });
  } else {
    try {
      runCommand('npx', ['@thearchitech/cli', 'new', '--genome', GENOME_PATH, 'synap-generated'], { cwd: OUTPUT_DIR });
    } catch (error) {
  ...
  const missingInGenerated = [...manualFiles].filter((file) => !generatedFiles.has(file)).sort();
  const newInGenerated = [...generatedFiles].filter((file) => !manualFiles.has(file)).sort();
