...
export default function InvestigatePage() {
  ...
  const { data, refetch: refetchEvents, isLoading } = useQuery({
    queryKey: ['events', 'search', searchTerm, eventTypeFilter],
    queryFn: () => AdminSDK.events.search({
      limit: 50,
      ...
      <Stack gap={spacing[6]}>
        ...
        <Tabs value={activeTab} onChange={setActiveTab}>
            ...
            <Stack gap="md">
              ...
              <Card padding="md" radius={borderRadius.lg} style={{ border: `1px solid ${colors.border.default}` }}>
                <Group gap="sm" align="flex-end">
                  ...
                  <Select
                    ...
                    data={capabilities?.eventTypes?.map((et: any) => ({ value: et.type, label: et.type })) || []}
              ...
              <Card padding="md" radius={borderRadius.lg} style={{ border: `1px solid ${colors.border.default}` }}>
                <Group justify="space-between" mb="md">
                  ...
                  <Badge variant="light" color="gray">{events?.length || 0} events</Badge>
                </Group>
                ...
                <ScrollArea style={{ height: '500px' }}>
                  <Stack gap="xs">
                    {isLoading ? (
                      ...
                      <Text size="sm" c="dimmed" ta="center" py="xl">No events found</Text>
                    ) : (
                      events.map((event) => (
                        <div
                          key={event.id}
                          ...
                          style={{
                            ...
                            backgroundColor: selectedEventId === event.id ? '#EFF6FF' : colors.background.secondary,
                            cursor: 'pointer',
                          }}
                        >
                          <Group justify="space-between" mb={4}>
                            <Badge variant="light" color="blue" size="sm">{event.type}</Badge>
                            <Text size="xs" c="dimmed">{new Date(event.timestamp).toLocaleString()}</Text>
                          </Group>
                          <Text size="xs" style={{ fontFamily: typography.fontFamily.mono }}>ID: {event.id}</Text>
                          {event.userId && <Text size="xs" c="dimmed">User: {event.userId}</Text>}
                        </div>
                      ))
                    )}
                  </Stack>
          ...
          <Tabs.Panel value="publish" pt="md">
            <Card padding="md" radius={borderRadius.lg} style={{ border: `1px solid ${colors.border.default}` }}>
              <Text size="lg" fw={600} mb="md">Publish Test Event</Text>
              <Stack gap="md">
                <Select
                  label="Event Type"
      ...
      <Drawer
        ...
        title={
          <Group justify="space-between" style={{ width: '100%' }}>
            <Text size="lg" fw={600}>Event Details & Trace</Text>
            {traceData?.event && (
              <Button 
                variant="light" 
                color="orange" 
      ...
      >
        {isLoadingTrace ? (
          ...
          <Stack gap="lg">
            {/* Main Event Details */}
            <div>
              <Text size="sm" fw={600} mb="sm">Main Event</Text>
              <Card padding="sm" style={{ backgroundColor: colors.background.secondary }}>
                <Stack gap="xs">
                  <Group gap="xs">
                    <IconTag size={16} color={colors.text.secondary} />
                    <Text size="sm" fw={500}>{traceData.event.eventType}</Text>
                  </Group>
                  <Group gap="xs">
                    <IconClock size={16} color={colors.text.secondary} />
                    <Text size="xs" c="dimmed">{new Date(traceData.event.timestamp).toLocaleString()}</Text>
                  </Group>
                  {traceData.event.userId && (
                    <Group gap="xs">
                      <IconUser size={16} color={colors.text.secondary} />
                      <Text size="xs" c="dimmed">{traceData.event.userId}</Text>
                    </Group>
                  )}
                  <Group gap="xs">
                    <IconCode size={16} color={colors.text.secondary} />
                    <Text size="xs" c="dimmed" style={{ fontFamily: typography.fontFamily.mono }}>{traceData.event.eventId}</Text>
                  </Group>
                </Stack>
              </Card>
            </div>
            ...
            {/* Event Data */}
            <div>
              <Text size="sm" fw={600} mb="sm">Event Data</Text>
            ...
            {traceData.relatedEvents && traceData.relatedEvents.length > 0 && (
              <div>
                <Group justify="space-between" mb="sm">
                  <Text size="sm" fw={600}>Related Events</Text>
                  <Badge variant="light" color="blue">{traceData.relatedEvents.length}</Badge>
                </Group>
                <Timeline active={-1} bulletSize={24} lineWidth={2}>
                  {traceData.relatedEvents.map((relEvent) => (
                    <Timeline.Item
                      ...
                      title={<Text size="sm" fw={500}>{relEvent.eventType}</Text>}
                    >
                      <Text size="xs" c="dimmed" mt={4}>{new Date(relEvent.timestamp).toLocaleString()}</Text>
                      <Text size="xs" c="gray" mt={2} style={{ fontFamily: typography.fontFamily.mono }}>{relEvent.eventId}</Text>
                      <Button
                        variant="subtle"
                        size="xs"
