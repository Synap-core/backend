...
import { 
  Card, Text, Stack, Group, Badge, Button, 
  ThemeIcon, Code, Collapse, TextInput,
  Alert
} from '@mantine/core';
import { 
  IconBolt, IconArrowRight, IconPlayerPlay, IconWebhook,
  IconCpu, IconChevronDown, IconChevronUp, IconCheck, IconX
...
export default function EventTypeExplorer({ eventType, onClose }: EventTypeExplorerProps) {
  ...
  const publishMutation = trpc.system.publishEvent.useMutation({
    onSuccess: (data) => {
      setPublishResult({ success: true, message: `Event published! ID: ${(data as any).eventId}` });
    },
    onError: (error) => {
  ...
  const subscribers = (capabilities as any)?.workers?.filter((w: any) => 
  ...
  return (
    <Stack gap="md">
      {/* Header */}
      <Card padding="md" radius={borderRadius.lg} style={{ background: colors.background.secondary }}>
        <Group justify="space-between">
          <Group>
            ...
            <div>
              <Text size="lg" fw={700}>{eventType}</Text>
              <Text size="sm" c="dimmed">Event Type Deep Dive</Text>
            </div>
          </Group>
      ...
      <Card padding="md" radius={borderRadius.lg} style={{ border: `1px solid ${colors.border.default}` }}>
        <Text size="sm" fw={600} mb="md">Flow Context</Text>
        <Group gap="xs" align="center" style={{ flexWrap: 'wrap' }}>
          <Badge size="lg" variant="light" color="gray" leftSection={<IconArrowRight size={12} />}>
            Trigger (API/SDK)
          </Badge>
          ...
          <Stack gap={4}>
            {subscribers.length > 0 ? (
              subscribers.map((s: any) => (
                <Badge key={s.name} size="lg" variant="light" color="violet" leftSection={<IconCpu size={12} />}>
                  {s.name}
                </Badge>
              ))
            ) : (
              <Badge size="lg" variant="light" color="gray">No workers</Badge>
            )}
            {subscribedWebhooks.map((wh: any) => (
              <Badge key={wh.id} size="lg" variant="light" color="green" leftSection={<IconWebhook size={12} />}>
                {wh.name}
              </Badge>
            ))}
          </Stack>
      ...
      {schemaData?.hasSchema && schemaData.fields && (
        <Card padding="md" radius={borderRadius.lg} style={{ border: `1px solid ${colors.border.default}` }}>
          <Text size="sm" fw={600} mb="md">Event Schema</Text>
          <Stack gap={4}>
            {schemaData.fields.map((field: any) => (
              <Group key={field.name} gap="xs">
                ...
                {field.required && <Badge size="xs" color="red">required</Badge>}
              </Group>
            ))}
          </Stack>
        </Card>
      ...
      <Card padding="md" radius={borderRadius.lg} style={{ border: `1px solid ${colors.border.default}` }}>
        <Group justify="space-between" mb={showPublish ? 'md' : 0}>
          <Text size="sm" fw={600}>Test: Publish Event</Text>
          <Button 
            variant="subtle" 
            size="xs"
            rightSection={showPublish ? <IconChevronUp size={14} /> : <IconChevronDown size={14} />}
        ...
        <Collapse in={showPublish}>
          <Stack gap="sm">
            ...
            {schemaData?.hasSchema ? (
              ...
              <Text size="xs" c="dimmed">No schema available. Using empty payload.</Text>
            )}
      ...
      <Card padding="md" radius={borderRadius.lg} style={{ border: `1px solid ${colors.border.default}` }}>
        <Group justify="space-between" mb="sm">
          <Text size="sm" fw={600}>Recent Instances</Text>
        ...
        <Stack gap={4}>
          {recentEvents?.events && recentEvents.events.length > 0 ? (
            recentEvents.events.slice(0, 5).map((event: any) => (
              <Group key={event.id} justify="space-between" style={{
            ...
            <Text size="xs" c="dimmed" ta="center" py="sm">No recent events</Text>
          )}
        </Stack>
      </Card>
    </Stack>
