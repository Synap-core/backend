 ...
 */
export const requestSizeLimit = async (c, next) => {
...
export const securityHeadersMiddleware = async (c, next) => {
    ...
    c.header('Content-Security-Policy', "default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; font-src 'self' data:; connect-src 'self' https://api.anthropic.com https://api.openai.com;");
