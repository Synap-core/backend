...
channelGatewayApp.post(
  "/verify",
  zValidator("json", verifySchema),
  async (c) => {
    const { token, channel, channelUserId, externalUsername } =
      c.req.valid("json");
      ...
      await tx
        ...
        .onConflictDoUpdate({
          target: [
            ...
            channelConnections.channelUserId,
          ],
          set: {
            userId: linkToken.userId,
            workspaceId: linkToken.workspaceId,
...
channelGatewayApp.post(
  ...
  async (c) => {
    const { channel, channelUserId, message, externalChatId, idempotencyKey } =
      c.req.valid("json");
    ...
    if (!connection) {
      return c.json(
        { error: "not_linked", message: "Account not linked" },
        404
      );
    }
...
channelGatewayApp.post(
  "/unlink",
  zValidator("json", unlinkSchema),
  async (c) => {
    const { channel, channelUserId } = c.req.valid("json");
