...
kratosWebhookRouter.post('/', async (c) => {
  try {
    ...
    console.log('[Webhook Debug] Received:', secret ? '${secret.substring(0,4)}...' : 'undefined');
    console.log('[Webhook Debug] Expected:', expectedSecret ? '${expectedSecret.substring(0,4)}...' : 'undefined');
    ...
    logger.info({ type: event.type, identityId: event.identity?.id }, 'Received Kratos webhook');
    ...
    if (event.type === 'identity.created' && event.identity) {
      ...
      logger.info({ identityId }, 'Successfully processed identity.created event');
    }
    ...
    if (event.type === 'identity.updated' && event.identity) {
      ...
      logger.info({ identityId }, 'Successfully processed identity.updated event');
    }
