...
Defaults to **`pod`** if not set on the profile (schema + write door). Only set `entityScope: "workspace"` for process kinds. The user can toggle this per profile in ProfileEditor Settings.
