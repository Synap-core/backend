 ...
 * gen-api-docs.mjs
 *
 * Generates MDX API reference files for the Synap backend tRPC routers.
 * Output: synap-team-docs/content/docs/api/index.mdx
 *
 * Usage:
 *   node scripts/gen-api-docs.mjs
 *   # or from monorepo root:
 *   pnpm --filter @synap/api gen-api-docs
 */
...
// synap-backend/packages/api → packages → synap-backend → synap root → synap-team-docs
const OUT_FILE = join(
  REAL_SCRIPT_DIR,
  "../../..",
  "synap-team-docs/content/docs/api/index.mdx"
);
...
function parseRouter(filePath) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Look for .output( on next few lines (optional)
    let outputShape = "";
    for (let j = i + 1; j < Math.min(i + 5, lines.length); j++) {
      const outputMatch = lines[j].match(/\.output\(([^)]{1,80})/);
      if (outputMatch) {
        outputShape = outputMatch[1].trim();
        break;
      }
    }
    ...
    procedures.push({ name, type, doc, inputShape, outputShape });
  }
...
/**
 * Format a procedure as MDX markdown.
 */
...
const output = `---
title: "API Reference"
description: "Auto-generated API reference for Synap backend tRPC procedures"
section: general
audience: developers
version: 1.0+
...
> **Auto-generated** from tRPC router definitions. Do not edit manually — run \`pnpm gen-api-docs\` in \`@synap/api\` to regenerate.
