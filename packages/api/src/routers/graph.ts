  ...
  getNode: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const whereClause = and(
        ...
        input.relationTypes ? inArray(relations.type, input.relationTypes) : undefined
      );
      ...
      const outgoing = allRelations.filter(r => r.sourceEntityId === input.entityId);
      const incoming = allRelations.filter(r => r.targetEntityId === input.entityId);
    ...
    .query(async ({ input, ctx }) => {
      if (input.entityId) {
        ...
        return {
          ...
          outgoing: relCount.filter(r => r.sourceEntityId === input.entityId).length,
          incoming: relCount.filter(r => r.targetEntityId === input.entityId).length,
        };
      }
      ...
      return {
        ...
        averageRelationsPerEntity: allEntities.length > 0 
          ? allRelations.length / allEntities.length 
          : 0,
      };
    }),
});
