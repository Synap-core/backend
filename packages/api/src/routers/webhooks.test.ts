...
describe('Webhooks Router', () => {
  ...
  describe('list', () => {
    it('should list user subscriptions without secrets', async () => {
      ...
      vi.mocked(db.query.webhookSubscriptions.findMany).mockResolvedValue(mockSubscriptions as any);
  ...
  describe('update', () => {
    it('should update subscription if owned by user', async () => {
      ...
      vi.mocked(db.query.webhookSubscriptions.findFirst).mockResolvedValue({ id: 'sub-1', userId: 'user-123' } as any);
    ...
    it('should throw NOT_FOUND if subscription does not exist or not owned', async () => {
      const { db } = await import('@synap/database');
      vi.mocked(db.query.webhookSubscriptions.findFirst).mockResolvedValue(undefined);
  ...
  describe('delete', () => {
    ...
    it('should throw NOT_FOUND if delete returns no rows', async () => {
      ...
      await expect(caller.delete({ id: 'sub-1' })).rejects.toThrow('Webhook subscription not found');
    });
  });
});
