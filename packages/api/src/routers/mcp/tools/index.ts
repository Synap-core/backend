  ...
  async list(ctx?: ToolsListContext): Promise<Tool[]> {
    const toolDefs: Tool[] = [
      ...
      },
      {
        ...
        description:
          "Update an in-flight focus session WHILE working: title (the name), goal, status (active|paused), progress, subject (`subjectEntityId` re-points what the work is ABOUT, null clears), project (`projectId` files the session into a project — always a proposal; null unfiles), deliverables (`addOutput` appends, `completeOutput` marks done by label), roster (`addAgentId` appends one agent, idempotently), and the playbook it FOLLOWS (`followPlaybookId` makes this session a run of that playbook and merges in its criteria + deliverables; null releases it). Cannot close — use synap_complete_session for that.",
        inputSchema: {
          type: "object",
          properties: {
            sessionId: {
      ...
      },
      {
        ...
        description:
          "Move a track to one of its method's stages — forward OR back (stages are re-enterable; any stage key synap_list_tracks shows). Refused for a stage the method does not declare, or a completed/archived track. A stage may carry a GATE: a human gate pauses the track and files an approval (`proposalId` on the reply), a check gate holds the track paused (pausedBy='check') until resumed with synap_set_track_status status='active'. Governed: status='proposed' is SUCCESS — pass on reviewUrl, do not retry.",
        inputSchema: {
          type: "object",
          properties: {
