  ...
  async list(ctx?: ToolsListContext): Promise<Tool[]> {
    const toolDefs: Tool[] = [
      ...
      },
      {
        ...
        description:
          'Get a document by ID: its markdown `content`, the `revision` to pass back as `baseRevision` when you edit it (synap_update_document), its top-level `sections` (id, owner ai|human, title — you may only rewrite `ai` ones), and `diagnostics` (embeds that will not render, each with a `fix`; null + diagnosticsError when they could not be checked). `format: "readable"` replaces every embed with its markdown fallback. Documents are long-form content (meeting notes, research, writeups, reports); get documentId from entity.documentId or search results.',
        inputSchema: {
          type: "object",
          properties: {
            ...
            format: {
              ...
              enum: ["raw", "readable"],
              description:
                "raw (default) = the stored markdown, embeds as directives (what you edit). readable = embeds replaced by their fallback text (what you quote or summarise).",
            },
          },
          required: ["documentId"],
        },
      ...
      },
      {
        ...
        description:
          "Read ONE proposal in full — the door for \"why did my write not land?\". Returns the row a reviewer sees: `status`, the whole `data` payload (the operations that would apply), `summary`, provenance, and — when an approve was attempted and FAILED (`status: 'approval_failed'`) — `rejectionReason` (the classified sentence) plus `data.failure` { errorClass, providerRef, missingFields }: the fields naming what to fix. Use it after a write comes back `proposed` and the user reports a problem approving it, then REPAIR the same proposal with synap_revise_proposal (never file a second one pointing at the first), or withdraw it with synap_reject_proposal. A proposal still `pending` after a failed approve attempt recorded nothing — read `data` and check the ops yourself. The raw upstream error text is deliberately NOT returned.",
        inputSchema: {
          type: "object",
          properties: {
            proposalId: {
              type: "string",
              description:
                "Proposal UUID, as returned verbatim by synap_list_proposals or by the `proposalId` of a `proposed` write. A leading fragment of one also resolves, but pass the full id you were given.",
            },
          },
          required: ["proposalId"],
        },
      ...
      },
      {
        ...
        description:
          "Update an entity's title, description, properties (JSONB), or long-form BODY (`content`). Requires entityId from search or synap_get_entity. May return 'proposed' if the write requires review. Use for status changes (task todo→done) and property updates. `content` REPLACES the whole body of the entity's linked document (an alias of synap_update_document with one replace_all op): it always goes to review, and it is refused if it changes a person's section or drops an embed. To change one section or one passage, use synap_update_document instead. If the entity has no body document yet the call is refused — create and attach one with synap_create_document({ entityId, title, content }).",
        inputSchema: {
          type: "object",
          properties: {
            ...
            content: {
              type: "string",
              description:
                "Full REPLACEMENT body (markdown) for the entity's linked document — not a patch (for a patch: synap_update_document). Read the current body with synap_get_document first. Its outcome comes back under `body` and is always 'proposed'.",
            },
            /**
             * WHY this write, in the person's own words — shown verbatim to the
             * reviewer. Without it the proposal detail reads "No reason was
      ...
      },
      {
        ...
        description:
          "Edit a document with ops — a section, a passage, an addition — instead of rewriting it. Read it first with synap_get_document and pass its `revision` as `baseRevision`. Ops, applied in order: `upsert_section {id, title, body}` writes ONE `::::synap-section` block by id (replaces your own `ai` section, or appends a new one); `replace_text {old, new}` — `old` must match EXACTLY once, copy it from the document and include enough context (0 or 2+ matches is refused with the count); `append {body}`; `replace_all {content}` (whole body — needs baseRevision, always goes to review). Refused: changing a section whose owner is `human` (by any op), and removing an embed unless `allow_removing_embeds: true`. Usually returns 'proposed' (the person reviews a per-section before/after); a governance rule may apply it directly. The response carries `preview` and `diagnostics` (embeds that will not render, each with a `fix` — advisory, never a refusal). A 'changed after you read it' error means re-read and redraft.",
        inputSchema: {
          type: "object",
          properties: {
            documentId: { type: "string", description: "Document UUID" },
            baseRevision: {
              type: "number",
              description:
                "The `revision` synap_get_document returned. The edit is refused if the document changed since.",
            },
            ops: {
              type: "array",
              minItems: 1,
              ...
              items: {
                ...
                properties: {
                  ...
                  id: {
                    type: "string",
                    description:
                      "upsert_section: the section id (lowercase letters, digits, '-' or '_', max 64).",
                  },
                  title: {
                    type: "string",
                    description: "upsert_section: the section heading.",
                  },
                  body: {
                    type: "string",
                    description: "upsert_section / append: markdown.",
                  },
                  status: {
                    type: "string",
                    description: "upsert_section: optional status stamp.",
                  },
                  old: {
                    type: "string",
                    description:
                      "replace_text: the exact text to replace — must occur exactly once.",
                  },
                  new: {
                    type: "string",
                    description: "replace_text: the replacement.",
                  },
                  content: {
                    type: "string",
                    description: "replace_all: the whole new body.",
                  },
                },
                required: ["op"],
              },
            },
            allow_removing_embeds: {
              type: "boolean",
              description:
                "Set true ONLY when removing an embed (synap-entity/view/cell) is the point of the edit; otherwise such an edit is refused.",
            },
            /**
             * WHY this write, in the person's own words — shown verbatim to the
             * reviewer. Without it the proposal detail reads "No reason was
      ...
      },
      {
        ...
        description:
          "Define a new entity KIND — AND the door for ADDING A FIELD to a kind that already exists. It is SLUG-IDEMPOTENT: re-call it with an EXISTING slug and it returns that profile and adds only the `properties[]` that are not there yet (an existing field reports status 'unchanged' and is never rewritten). So to add `wedge_stage` to `question`, call this with slug 'question' and just that one field — there is NO separate add-property tool, and schema editing is NOT browser-only. A field takes `valueType`, `constraints` (an enum of allowed values lives here), `defaultValue`, `required`, and `overlay: true` to add it as a WORKSPACE OVERLAY instead of a base field on the kind — which is what you want for a field that only one domain should see, on a pod-wide kind like note/task/question. Call list_profiles + extend-first (load_skill system/synap-schema/extend-first) BEFORE minting a new slug. A hat on an existing kind — including item, task, deal — is a ROLE (synap_define_role / attach_facet), never a sibling kind. Prefer parentProfileSlug of the closest parent over a disconnected slug. A relationship-with-its-own-life (buyer×seller×price×stage) is its own kind (deal *precedent*), not a twin of CRM deal. POD-WIDE default. Governed: 'proposed' is success.",
        inputSchema: {
          type: "object",
          properties: {
            slug: {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            trackId: {
              type: "string",
              description:
                "Optional track UUID (synap_list_tracks) — the method, inside a project, this session advances. Implies its project; a different projectId is refused.",
            },
            trackStage: {
              type: "string",
              description:
                "Optional stage KEY of that track the session is filed at. Omit to file it at the track's current stage; a key the track does not declare is refused. Needs trackId. To start a stage's own session (its goal, outputs and criteria), prefer synap_start_stage_session.",
            },
            subjectEntityId: {
              type: "string",
              description:
            ...
            channelId: {
              type: "string",
              description:
                "Optional channel UUID for the session to BORROW as its room (e.g. the chat it started in). Omit it and the session mints its own group room, returned as `channelId` — ask the person there.",
            },
            agentIds: {
              type: "array",
              items: { type: "string" },
      ...
      },
      {
        ...
        description:
          "End a focus session — the ONE door for every lifecycle exit. `terminalStatus` says HOW it ended: 'closed' (the work finished, default), 'cancelled' (abandoned — you stopped on purpose), 'failed' (you could not complete it). Say which honestly: a cancelled or failed session recorded as 'closed' tells the user work succeeded when it did not. 'cancelled' also STOPS the session's in-flight work where a real stop exists (queued jobs, a running agent reply) and returns `cancel`: { stopped[], notStoppable[] (already running — it will finish), finished[] (proposals that already applied) }. A cancel may come back status 'proposed' — that is NOT an error: it is queued for the user's review and nothing is stopped until they approve. Also closes any running playbook_run. Returns a **review pack**: pendingProposals[], counts, and warnings (e.g. unfinished expectedOutputs — warn only), plus `nudges` for what closed still owed (ungraded criteria: grade them with synap_evaluate_session — best BEFORE closing). Use synap_list_proposals({sessionId}) to re-fetch the pack.",
        inputSchema: {
          type: "object",
          properties: {
            sessionId: {
      ...
      },
      {
        ...
        description:
          "List the projects (cross-cutting initiatives) you can see, pod-wide, newest first — call this to find a project id before reading, updating or linking it. Each row: id, name, description, status, phase, phaseCategory (legacy label; method position lives on tracks: synap_list_tracks), targetDate, homeWorkspaceId, usedWorkspaceIds (domains it runs through) and its subject. Paginated: { items, pagination: { hasMore, limit, offset } }. Full read: synap_get_project.",
        inputSchema: {
          type: "object",
          properties: {
            status: {
      ...
      },
      {
        ...
        description:
          "List a project's TRACKS — the methods it is running (e.g. 'Business model', 'Content', 'Build' inside one project). Each track follows one project-scoped playbook (its method), pinned at the version it started with. Each row: id, name, playbookId, methodVersion, status (active|paused|completed|archived), pausedBy (check|human|null — why a paused track is held), currentStage and stages [{ key, name, position: done|active|not_started }]. Archived tracks are omitted unless includeArchived. Find the project id with synap_list_projects.",
        inputSchema: {
          type: "object",
          properties: {
            projectId: { type: "string", description: "Project UUID" },
            includeArchived: {
              type: "boolean",
              description: "Also list archived tracks. Default false.",
            },
          },
          required: ["projectId"],
        },
      },
      {
        ...
        description:
          "Start a METHOD on a project as a new track. The method is a playbook with scope 'project' (find one with synap_list_playbooks); a session-scoped playbook is REFUSED — start it as a session instead. Idempotent: if that method already runs on the project, the existing track comes back with status='exists'. Governed: status='proposed' is SUCCESS — say it awaits review, pass on reviewUrl, do not retry. Otherwise status='started' with the track.",
        inputSchema: {
          type: "object",
          properties: {
            projectId: { type: "string", description: "Project UUID" },
            playbookId: {
              type: "string",
              description: "UUID of a project-scoped playbook (the method).",
            },
            name: {
              type: "string",
              description:
                "Optional display name; defaults to the method's name.",
            },
            params: {
              type: "object",
              description:
                "Optional answers to the method's declared params (its onboarding), by param name. A mistyped value or an unknown name is refused. A REQUIRED param you leave out is not refused: it becomes a question owed to the person on the stage session that needs it.",
            },
            reasoning: {
              type: "string",
              description:
                "Why you are starting it, in the person's words — shown to the reviewer. One line.",
            },
          },
          required: ["projectId", "playbookId"],
        },
      },
      {
        name: "synap_start_stage_session",
        annotations: {
          title: "Start stage session",
          readOnlyHint: false,
          destructiveHint: false,
          openWorldHint: false,
        },
        description:
          "Start the work session for ONE stage of a track — the session a stage OFFERS when the track enters it (synap_advance_track returns it as `offer`). Advancing a track never starts work; call this only when the person wants that step worked on now. Defaults to the track's CURRENT stage; the stage's goal is the session's brief, its expected outputs and criteria are copied on, and the track's params are passed, so a required param nobody answered becomes a question owed to the person. Idempotent: an open session already filed at that stage comes back with status='existing'. Governed: status='proposed' is SUCCESS — say it awaits review, pass on reviewUrl, do not retry.",
        inputSchema: {
          type: "object",
          properties: {
            ...
            stageKey: {
              type: "string",
              description:
                "The stage KEY to work on. Omit for the track's current stage.",
            },
            title: {
              type: "string",
              description:
                "Optional one-line session name; defaults to the stage's name.",
            },
            goal: {
              type: "string",
              description:
                "Optional outcome in the person's words; defaults to the stage's own goal.",
            },
          },
          required: ["trackId"],
        },
      },
      {
        ...
        description:
          "Move a track to one of its method's stages — forward OR back (stages are re-enterable; any stage key synap_list_tracks shows). Refused for a stage the method does not declare, or a completed/archived track. A stage may carry a GATE: a human gate pauses the track and files an approval (`proposalId` on the reply); a check gate measures the sessions filed at the stage being left (at least one closed, every closed one passing its criteria) and otherwise holds the track paused (pausedBy='check', `check.reason` says why) until resumed with synap_set_track_status status='active'. Entering a stage does NOT start work: the reply's `offer` is the session that stage suggests — offer it to the person, and start it with synap_start_stage_session only if they want it. Governed: status='proposed' is SUCCESS — pass on reviewUrl, do not retry.",
        inputSchema: {
          type: "object",
          properties: {
            trackId: { type: "string", description: "Track UUID" },
            toStage: {
              type: "string",
              description: "The stage KEY to move to (not its display name).",
            },
            reasoning: {
              type: "string",
              description:
                "Why the work moves to this stage — shown to the reviewer. One line.",
            },
          },
          required: ["trackId", "toStage"],
        },
      },
      {
        name: "synap_set_track_status",
        annotations: {
          title: "Set track status",
          readOnlyHint: false,
          destructiveHint: false,
          openWorldHint: false,
        },
        description:
          "Pause, resume, complete, reopen or archive a track. Allowed moves: active → paused|completed|archived; paused → active|completed|archived; completed → active|archived; archived is FINAL (start the method again with synap_start_track). Resuming (status='active') releases a track a check gate held (pausedBy='check'). A move the table does not allow is refused with the reason. Governed: status='proposed' is SUCCESS — pass on reviewUrl, do not retry.",
        inputSchema: {
          type: "object",
          properties: {
            trackId: { type: "string", description: "Track UUID" },
            status: {
              type: "string",
              enum: [...PROJECT_TRACK_STATUSES],
              description: "The status to move the track to.",
            },
            reasoning: {
              type: "string",
              description:
                "Why the track changes status — shown to the reviewer. One line.",
            },
          },
      ...
      {
        name: "synap_set_track_params",
        annotations: {
          title: "Set track params",
          readOnlyHint: false,
          destructiveHint: false,
          openWorldHint: false,
        },
        description:
          "Answer (some of) a track's method params — the questions the method DECLARED when the track started (synap_list_tracks returns them as `declaredParams`, with each one's `type`, `required` and any `options`). MERGED onto the answers already given: only the keys you pass change, and a null value clears that answer. A key the method does not declare, or a value of the wrong type, is refused with the reason. Stage sessions started afterwards carry the answers; a required param still unanswered is a question owed to the person. Governed: status='proposed' is SUCCESS — pass on reviewUrl, do not retry.",
        inputSchema: {
          type: "object",
          properties: {
            trackId: { type: "string", description: "Track UUID" },
            params: {
              ...
              additionalProperties: true,
              description:
                "Answers keyed by the declared param `name`. null clears an answer.",
            },
            reasoning: {
              type: "string",
              description:
                "Why these answers — shown to the reviewer. One line.",
            },
          },
        ...
        inputSchema: {
          ...
          properties: {
            ...
            phase: {
              type: ["string", "null"],
              description:
                "Legacy lifecycle label (free text, ≤120 chars). Superseded by tracks for method position: use synap_list_tracks / synap_advance_track. null clears it; omit to leave it.",
            },
            targetDate: {
              type: ["string", "null"],
              description:
      ...
      },
      {
        ...
        description: `Create a view in a workspace (recovery when the right view is missing, or proactive once data warrants it). Call synap_list_views first — don't duplicate. For a bento, call synap_list_widgets first and only place keys from that list (stat-card needs profileSlug; view needs a saved viewId, not a profileSlug). Type: one of ${CREATABLE_VIEW_TYPE_KEYS.join(", ")} (sheet is the table door). profileId scopes to one entity type. For a whiteboard, pass initialContent to seed the shapes — it is the ONLY way to put content on a board, because content cannot be updated after create. Governed: may propose. On success the result includes \`link\` (\`\${PUBLIC_URL}/open/<id>\`) — surface that URL to the user.`,
        inputSchema: {
          type: "object",
          properties: {
            name: { type: "string" },
        ...
        inputSchema: {
          ...
          properties: {
            surface: {
              ...
              enum: ["bento", "document"],
              description:
                "Where you will place it: bento (dashboard block, default) or document (an embed inside a document).",
            },
            workspaceId: {
              type: "string",
              description:
        ...
        inputSchema: {
          ...
          properties: {
            ...
            projectId: {
              type: "string",
              description:
                "Optional project UUID to file the run into. Must be a project you can see.",
            },
            trackId: {
              type: "string",
              description:
                "Optional track UUID (synap_list_tracks) — the method, inside a project, this run advances. Implies its project; a different projectId is refused.",
            },
          },
          required: [],
        },
