...
export function registerWorkspacesRoutes(app: HubHono): void {
  ...
  /**
   * PATCH /workspaces/:workspaceId/governance
   *
   * Set the agent governance policy for this workspace (autoApproveFor +
   * writesRequireProposal). Owner/admin only — same membership gate as
   * delivery-preferences.
   */
  app.patch("/workspaces/:workspaceId/governance", async (c) => {
    if (!hasScope(c.get("scopes") as string[], "hub-protocol.write")) {
      return c.json(
        { error: "Insufficient scope: hub-protocol.write required" },
        403
    ...
    try {
      ...
      if (
        ...
        return c.json(
          { error: "Only owners and admins can configure governance" },
          403
        );
      }
      ...
      const currentAiGovernance =
        ((existing?.settings ?? {}) as Record<string, unknown>).aiGovernance ??
        {};
      const merged = {
        ...(currentAiGovernance as Record<string, unknown>),
      ...
      return c.json({ ok: true, workspaceId, aiGovernance: merged });
    } catch (err) {
      logger.error(
        { err, userId, workspaceId },
        "PATCH /workspaces/:workspaceId/governance failed"
      );
      return c.json({ error: "Failed to apply governance" }, 500);
    }
  });
