...
export const approvalsRouter = router({
  ...
  stats: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      return {
        pending: parseInt(stats.rows.find((r: any) => r.status === 'pending')?.count || '0'),
        approved: parseInt(stats.rows.find((r: any) => r.status === 'approved')?.count || '0'),
        denied: parseInt(stats.rows.find((r: any) => r.status === 'denied')?.count || '0'),
      };
    }),
});
