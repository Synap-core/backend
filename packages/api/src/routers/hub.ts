...
async function getCalendar(userId: string): Promise<Array<Record<string, unknown>>> {
  logger.debug({ userId }, 'Retrieving calendar (placeholder)');
  return [];
}

async function getNotes(userId: string, filters?: { dateRange?: { start: string; end: string }; limit?: number; offset?: number }): Promise<Array<Record<string, unknown>>> {
  ...
  if (filters?.dateRange) {
    ...
    whereCondition = and(whereCondition, gte(entities.createdAt, startDate) as any, lte(entities.createdAt, endDate) as any) as any;
  }
  ...
  }).from(entities).where(whereCondition).orderBy(desc(entities.createdAt) as any).limit(filters?.limit || 100).offset(filters?.offset || 0);
...
async function getTasks(userId: string, filters?: { dateRange?: { start: string; end: string }; limit?: number; offset?: number }): Promise<Array<Record<string, unknown>>> {
...
async function getProjects(userId: string, filters?: { dateRange?: { start: string; end: string }; limit?: number; offset?: number }): Promise<Array<Record<string, unknown>>> {
  logger.debug({ userId, filters }, 'Retrieving projects');
  let whereCondition: SQL | undefined = eq(entities.userId, userId) as any;
  whereCondition = and(whereCondition, eq(entities.type, 'project') as any) as any;
  if (filters?.dateRange) {
    const startDate = new Date(filters.dateRange.start);
    const endDate = new Date(filters.dateRange.end);
    whereCondition = and(whereCondition, gte(entities.createdAt, startDate) as any, lte(entities.createdAt, endDate) as any) as any;
...
async function getConversations(userId: string): Promise<Array<Record<string, unknown>>> {
  logger.debug({ userId }, 'Retrieving conversations (placeholder)');
  return [];
}

async function getAllEntities(userId: string, filters?: { dateRange?: { start: string; end: string }; entityTypes?: string[]; limit?: number; offset?: number }): Promise<Array<Record<string, unknown>>> {
  logger.debug({ userId, filters }, 'Retrieving all entities');
  let whereCondition: SQL | undefined = eq(entities.userId, userId) as any;
  if (filters?.entityTypes && filters.entityTypes.length > 0) {
    whereCondition = and(whereCondition, eq(entities.type, filters.entityTypes[0]) as any) as any;
  }
  if (filters?.dateRange) {
    const startDate = new Date(filters.dateRange.start);
    const endDate = new Date(filters.dateRange.end);
...
async function getRelations(userId: string): Promise<Array<Record<string, unknown>>> {
  logger.debug({ userId }, 'Retrieving relations (placeholder)');
  return [];
}

async function getKnowledgeFacts(userId: string): Promise<Array<Record<string, unknown>>> {
...
export const hubRouter = router({
  generateAccessToken: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      logger.info({ userId, requestId, scope, expiresIn }, 'Generating Hub access token');
      const { token, expiresAt } = generateHubAccessToken(userId, requestId, scope, expiresIn);
  ...
  requestData: hubTokenProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const invalidScopes = requestedScope.filter(s => !tokenScope.includes(s));
  ...
  submitInsight: hubTokenProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (insight.type === 'action_plan' || insight.type === 'automation') {
        ...
        logger.debug({ userId, requestId, insightType: insight.type }, 'Insight type does not create events');
      }
