...
describe('ApiKeyService', () => {
  ...
  describe('generateApiKey', () => {
    ...
    it('should set expiration date when expiresInDays is provided', async () => {
      ...
      const diff = Math.abs(storedKey.expiresAt!.getTime() - expected.getTime());
    ...
    it('should set rotation scheduled date (90 days)', async () => {
      ...
      const diff = Math.abs(storedKey.rotationScheduledAt!.getTime() - expected.getTime());
      expect(diff).toBeLessThan(1000); // Within 1 second
    });
  });
  ...
  describe('rotateApiKey', () => {
    it('should create a new key and revoke the old one', async () => {
      ...
      const { key: oldKey, keyId: oldKeyId } = await apiKeyService.generateApiKey(
        testUserId,
        'Original Key',
        ['preferences', 'notes'],
        testHubId
      ...
      // Rotate
      const { newKey, newKeyId } = await apiKeyService.rotateApiKey(oldKeyId, testUserId);
    ...
    it('should throw error if key not found', async () => {
      await expect(
        apiKeyService.rotateApiKey('00000000-0000-0000-0000-000000000000', testUserId)
