...
describe("verbType — builtin read classification derives from the execute gate", () => {
  ...
  it("does not classify a non-builtin skill as read merely by sharing a name with a builtin verb", () => {
    // "graph.relations" is a READ_ONLY_BUILTIN_VERBS member whose tokens
    // ("graph", "relations") match neither READ_TOKEN nor ACTION_TOKEN, so a
    // non-builtin skill of the same name must fall through to the "write"
    // default — the gate-derived fast path must require kind === "builtin".
    expect(verbType("graph.relations", null, "declarative")).toBe("write");
    expect(verbType("graph.relations", null, "builtin")).toBe("read");
  });
});
