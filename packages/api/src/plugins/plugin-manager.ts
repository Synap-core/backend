...
class PluginManager {
  ...
  async register(plugin: DataPodPlugin): Promise<void> {
    ...
    if (this.initialized && plugin.onInit) {
      try {
        ...
        logger.error({ err: error, plugin: plugin.name }, 'Failed to initialize plugin');
        throw error;
      }
    }
    ...
    logger.info({ plugin: plugin.name, version: plugin.version }, 'Plugin registered');
  }
  ...
  async processThought(input: ThoughtInput): Promise<ThoughtResponse | null> {
    const enabledPlugins = this.getEnabledPlugins().filter(p => p.processThought);
    ...
    for (const plugin of enabledPlugins) {
      ...
      try {
        logger.debug({ plugin: plugin.name, userId: input.userId }, 'Trying plugin for thought processing');
        const result = await plugin.processThought(input);
        
        if (result.success) {
          logger.info({ plugin: plugin.name, requestId: result.requestId }, 'Plugin processed thought successfully');
          return result;
        }
      } catch (error) {
        logger.warn({ err: error, plugin: plugin.name }, 'Plugin processing failed, trying next');
