...
import { userPreferences, workspaces, intelligenceServices } from '@synap/database/schema';
...
export async function resolveIntelligenceService(
  ...
  const wsOverrides = wsSettings?.intelligenceServiceOverrides as Record<string, string> | undefined;
  const wsServiceId = (capability !== 'default' && wsOverrides?.[capability]) || wsSettings?.intelligenceServiceId;
  ...
  const userServicePrefs = (userPrefs?.intelligenceServicePreferences as Record<string, string> | undefined) || {};
  const userServiceId = userServicePrefs[capability] || userServicePrefs.default;
