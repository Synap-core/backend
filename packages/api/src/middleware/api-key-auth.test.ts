...
const createTestRouter = () => {
  ...
  const scopedProc = (scopes: string[]) => protectedProc.use(createScopedProcedure(scopes));
...
describe('API Key Middleware', () => {
  ...
  it('should throw UNAUTHORIZED if API key is invalid', async () => {
    ...
    await expect(caller.testProtected()).rejects.toThrow('Invalid or expired API key');
  });
  ...
  it('should deny access if required scope is missing', async () => {
    ...
    await expect(caller.testScoped()).rejects.toThrow('Insufficient permissions');
  });
});
