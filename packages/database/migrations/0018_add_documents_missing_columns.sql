...
-- Description: Adds columns that exist in Drizzle schema but missing in database
-- This migration adds all columns needed for the full documents schema
-- 
...
BEGIN
  ...
  -- Add storage_url column (nullable)
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
  ...
  -- Add storage_key column (nullable)
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
