...
export const entityVectors = pgTable(
  "entity_vectors",
  {
  ...
  (table) => ({
    userIdIdx: index("entity_vectors_user_id_idx").on(table.userId),
    // HNSW index for cosine ANN search defined in migration SQL (Drizzle doesn't support HNSW syntax yet)
  })
);
