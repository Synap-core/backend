...
describe('DocumentRepository', () => {
  ...
  beforeEach(() => {
    ...
    mockDb.insert.mockReturnValue({
      values: vi.fn().mockReturnValue({
        returning: vi.fn().mockResolvedValue([{ id: 'doc-1', title: 'Test Document' }])
      })
    });
    ...
    mockDb.update.mockReturnValue({
      set: vi.fn().mockReturnValue({
        where: vi.fn().mockReturnValue({
          returning: vi.fn().mockResolvedValue([{ id: 'doc-1', title: 'Updated Document' }])
        })
      })
    });
  ...
  describe('update', () => {
    it('should update document and emit completed event', async () => {
      const updated = await docRepo.update('doc-1', {
