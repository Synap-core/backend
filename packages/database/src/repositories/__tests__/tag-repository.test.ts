...
describe('TagRepository', () => {
  ...
  beforeEach(() => {
    ...
    mockDb.insert.mockReturnValue({
      values: vi.fn().mockReturnValue({
        returning: vi.fn().mockResolvedValue([{ id: 'tag-1', name: 'Test', color: '#blue' }])
      })
    });
    ...
    mockDb.update.mockReturnValue({
      set: vi.fn().mockReturnValue({
        where: vi.fn().mockReturnValue({
          returning: vi.fn().mockResolvedValue([{ id: 'tag-1', name: 'Updated', color: '#red' }])
        })
      })
    });
  ...
  describe('create', () => {
    ...
    it('should throw if tag already exists', async () => {
      ...
      await expect(tagRepo.create({
        name: 'Test',
        color: '#blue',
        userId: 'user-1',
      }, 'user-1')).rejects.toThrow(/Tag with name "Test" already exists/);
    });
  });

  describe('update', () => {
    it('should update tag and emit completed event', async () => {
      const updated = await tagRepo.update('tag-1', {
        name: 'Updated',
        color: '#red',
      }, 'user-1');
