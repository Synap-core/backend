...
export class InboxRepository extends BaseRepository<InboxItem, CreateInboxItemInput, UpdateInboxItemInput> {
  constructor(db: any, eventRepo: EventRepository) {
    super(db, eventRepo, { subjectType: 'inbox_item', pluralName: 'inbox_items' });
  }
   ...
   */
  async update(id: string, data: UpdateInboxItemInput, userId: string): Promise<InboxItem> {
    const [item] = await this.db
      .update(inboxItems)
      .set({
        title: data.title,
