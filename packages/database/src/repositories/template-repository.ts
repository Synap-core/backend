...
import type { EntityTemplate, NewEntityTemplate } from '../schema/entity-templates.js';
...
export class TemplateRepository extends BaseRepository<EntityTemplate, CreateTemplateInput, UpdateTemplateInput> {
   ...
   */
  async create(data: CreateTemplateInput, userId: string): Promise<EntityTemplate> {
   ...
   */
  async update(id: string, data: UpdateTemplateInput, userId: string): Promise<EntityTemplate> {
    const [template] = await this.db
      ...
      .where(and(eq(entityTemplates.id, id), eq(entityTemplates.userId, userId)))
      .returning();
