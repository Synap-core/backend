...
export const workerRegistry: WorkerMetadata[] = [
  ...
  // ============================================================================
  {
    ...
    description: 'Handles all entity CRUD operations (notes, tasks, projects, etc.)',
    triggers: [
 ...
 */
export function getWorkersByCategory(category: WorkerMetadata['category']): WorkerMetadata[] {
...
export function getWorkersForEvent(eventType: string): WorkerMetadata[] {
  return workerRegistry.filter(w => 
    w.triggers.includes(eventType) || w.triggers.includes('*')
  );
}
