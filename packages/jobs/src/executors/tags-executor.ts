...
export const tagsHandler = async ({ event, step }: { event: any; step: any }) => {
  ...
  const action = event.name.split('.')[1] as 'create' | 'update' | 'delete' | 'attach' | 'detach';
