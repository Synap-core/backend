...
import { getDb, EventRepository, EntityRepository, DocumentRepository, eq, and, entities, documents } from "@synap/database";
import { randomUUID } from "crypto";

export const entitiesHandler = async ({ event, step }: { event: any, step: any }) => {
      ...
      if (entity?.documentId) {
        await step.run('delete-linked-document', async () => {
          ...
          const document = await db.query.documents.findFirst({
            where: and(eq(documents.id, entity.documentId!), eq(documents.userId, userId))
          });
