...
const thoughtAnalysisSchema = z.object({
  title: z.string().describe('A concise title for this thought (max 100 chars)'),
  tags: z.array(z.string()).describe('Relevant tags (3-5 tags)'),
  intent: z.enum(['note', 'task', 'event', 'idea']).describe('What type of entity is this?'),
  dueDate: z.string().optional().describe('ISO date if this is a task (e.g., "2025-11-06")'),
  priority: z.number().min(0).max(3).optional().describe('Priority for tasks: 0=none, 1=low, 2=medium, 3=high'),
});
