...
export const workspaceMembersWorker = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    logger.info({ eventName, action, userId }, 'Processing workspace member operation');
    ...
    if (action === 'add') {
      const member = await step.run('add-member', async () => {
        ...
        logger.info(
          { workspaceId: event.data.workspaceId, targetUserId: event.data.targetUserId, role: member.role },
          'Member added'
        );
      ...
      if (event.data.inviteId) {
        await step.run('delete-invite', async () => {
          const { getDb, workspaceInvites, eq } = await import('@synap/database');
          const db = await getDb();
          
          await db.delete(workspaceInvites).where(eq(workspaceInvites.id, event.data.inviteId));
    ...
    if (action === 'remove') {
      await step.run('remove-member', async () => {
        const { getDb, workspaceMembers, and, eq } = await import('@synap/database');
        const db = await getDb();
        ...
        logger.info(
          { workspaceId: event.data.workspaceId, targetUserId: event.data.targetUserId },
          'Member removed'
        );
      });
    ...
    if (action === 'updateRole') {
      const member = await step.run('update-role', async () => {
        ...
        logger.info(
          { workspaceId: event.data.workspaceId, targetUserId: event.data.targetUserId, newRole: event.data.newRole },
          'Member role updated'
        );
