...
export const workspacesWorker = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    logger.info({ eventName, action, userId }, 'Processing workspace operation');
    ...
    if (action === 'create') {
      ...
      await step.run('add-owner-member', async () => {
        ...
        logger.info({ workspaceId: workspace.id, userId }, 'Owner member added');
      });
      ...
      await step.run('emit-created', async () => {
        ...
        logger.info({ workspaceId: workspace.id }, 'Create completed event emitted');
      });
    ...
    if (action === 'update') {
      const workspace = await step.run('update-workspace', async () => {
        ...
        if (event.data.description !== undefined) updateData.description = event.data.description;
    ...
    if (action === 'delete') {
      await step.run('delete-workspace', async () => {
        ...
        logger.info({ workspaceId: event.data.workspaceId }, 'Workspace deleted');
      });
