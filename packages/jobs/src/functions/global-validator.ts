...
export const globalValidator = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const source = event.data.source || (event.data.metadata as any)?.source || 'user';
    ...
    logger.info({ eventName, userId, action, targetType, source }, 'Validating request');
    ...
    if (!permissionResult.granted) {
      // Emit Denied
      const denialReason = (permissionResult as any).reason || 'Permission denied';
    ...
    const policyResult = await step.run('check-policy', async () => {
      ...
      if (source === 'ai') {
        ...
        const [ws] = workspaceId ? await db.select().from(workspaces).where(eq(workspaces.id, workspaceId)).limit(1) : [];
