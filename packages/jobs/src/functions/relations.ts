  ...
  async ({ event, step }) => {
    ...
    if (action === 'create') {
      ...
      await step.run('verify-entity-ownership', async () => {
        const { getDb, entities, eq, or, and } = await import('@synap/database');
    ...
    if (action === 'delete') {
      ...
      await step.run('verify-relation-ownership', async () => {
        ...
        logger.debug({ relationId: data.relationId, userId }, 'Relation ownership verified');
      });
      ...
      await step.run('delete-relation', async () => {
        ...
        await db.delete(relations).where(and(
          eq(relations.id, data.relationId),
          eq(relations.userId, userId)
        ));
      ...
      await step.run('emit-delete-validation', async () => {
        ...
        logger.info({ relationId: data.relationId }, 'Published relations.delete.validated');
      });
