    ...
    if (action === "create") {
      ...
      if (data.content || data.file) {
        fileInfo = await step.run("upload-file", async () => {
          ...
          logger.debug(
            { storagePath, size: buffer.length },
            "Uploaded entity file",
          );
      ...
      await step.run("insert-entity", async () => {
        ...
        try {
          ...
          logger.info(
            { entityType, dataEntityType: data.entityType },
            "🏷️ Entity type",
          );
      ...
      await step.run("emit-socketio", async () => {
        const REALTIME_URL =
          process.env.REALTIME_URL || "http://localhost:3001";
