    ...
    const results = await Promise.all(activeSessions.map(async (session) => {
      try {
        ...
        if (!response.ok) {
          ...
          if (response.status === 404) return { id: session.documentId, status: 'skipped-no-room' };
        ...
        logger.error({ error: e, documentId: session.documentId }, 'Failed to persist document state');
        return { id: session.documentId, error: e instanceof Error ? e.message : 'Unknown error' };
      }
    }));
