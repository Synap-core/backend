  ...
  async ({ event, step }) => {
    ...
    if (action === 'create') {
      ...
      const fileInfo = await step.run('upload-document', async () => {
        ...
        const ext = data.file?.filename?.split('.').pop() || 
      ...
      await step.run('create-initial-version', async () => {
        ...
        const content = data.file?.content || data.content || '';
        const textContent = data.file?.source === 'file-upload'
    ...
    if (action === 'update') {
      ...
      const currentVersion = await step.run('get-current-version', async () => {
        const { getDb, documents: docsTable, eq } = await import('@synap/database');
        const db = await getDb();
        
        const [doc] = await db.select({ currentVersion: docsTable.currentVersion })
          .from(docsTable)
      ...
      await step.run('update-storage', async () => {
        ...
        const [doc] = await db.select({ storageKey: docsTable.storageKey })
          .from(docsTable)
          .where(eq(docsTable.id, documentId))
          .limit(1);
      ...
      await step.run('update-document', async () => {
        const { getDb, documents: docsTable, eq, and } = await import('@synap/database');
        const db = await getDb();
