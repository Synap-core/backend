...
describeIf('User Isolation (Application-Level Filtering)', () => {
  ...
  it('should filter events by userId', async () => {
    ...
    // User A queries
    const eventsA = await db.select()
      .from(events)
      .where(eq(events.userId, userA));

    // User B queries
    const eventsB = await db.select()
      .from(events)
  ...
  it('should filter entities by userId', async () => {
    ...
    const [entityB] = await db.insert(entities).values({
      type: 'note',
      title: 'User B Note',
      userId: userB,
    }).returning();
    ...
    // User A queries
    const entitiesA = await db.select()
      .from(entities)
      .where(eq(entities.userId, userA));

    // User B queries
    const entitiesB = await db.select()
      .from(entities)
  ...
  it('should create user-scoped tags', async () => {
    ...
    const [tagB] = await db.insert(tags).values({
      name: 'important',
      userId: userB,
    }).returning();
  ...
  it('should filter tags by userId', async () => {
    // User A queries tags
    const tagsA = await db.select()
      .from(tags)
      .where(eq(tags.userId, userA));

    // User B queries tags
    const tagsB = await db.select()
      .from(tags)
      .where(eq(tags.userId, userB));
  ...
  it('should isolate search results by userId', async () => {
    // User A searches
    const resultsA = await db.select()
      .from(entities)
      .where(eq(entities.userId, userA));

    // User B searches
    const resultsB = await db.select()
      .from(entities)
      .where(eq(entities.userId, userB));
