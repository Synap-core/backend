...
async function runTests() {
  ...
  try {
    ...
    logTest('Test 4: Row-Level Security (RLS)', false, (error as Error).message);
  }
  ...
  try {
    ...
    const response = await fetch(`${API_URL}/api/trpc/events.list?input=${encodeURIComponent(JSON.stringify({ limit: 10 }))}`);
    
    // Even if unauthorized, we should get a proper tRPC error response
    const isValidTRPCResponse = response.headers.get('content-type')?.includes('json');
  ...
  try {
    ...
    logTest('Test 7: Better Auth Routes', response.status === 200 || response.status === 401);
  } catch (error) {
