...
export class CollaborationManager {
  ...
  userJoined(params: {
    ...
    this.io.to(`view:${params.viewId}`).except(params.socketId).emit('user:joined', {
  ...
  updateCursor(params: {
    ...
    if (session) {
      ...
      this.io.to(`view:${params.viewId}`).except(params.socketId).emit('cursor:update', {
  ...
  setTyping(params: {
    ...
    if (session) {
      ...
      this.io.to(`view:${params.viewId}`).except(params.socketId).emit('user:typing', {
        userId: session.userId,
  ...
  getStats() {
    return {
      ...
      sessionsByView: Array.from(this.presenceManager.getAllSessions())
        .reduce((acc, session) => {
