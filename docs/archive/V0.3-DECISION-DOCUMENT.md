...
sequenceDiagram
    ...
    participant API as API (Hono/tRPC)
    participant ChatDB as Chat History DB
    participant EventStore as Event Store
    participant AI as AI Service (OpenAI)
    participant StateDB as State DB (SQL Projection)
    participant Inngest as Inngest Worker

    User->>Frontend: Tape "Idée : une app de notes intelligente"
    Frontend->>API: POST /api/chat/send_message { threadId, content }
    API->>ChatDB: INSERT new 'user' message

    API-->>Frontend: Acknowledge (message reçu)

    %% Le processus IA commence %%
    API->>AI: Analyse le message + l'historique du chat
    AI-->>API: Réponse: "Bonne idée. Actions possibles: [Créer Note], [Créer Projet]"

    API->>ChatDB: INSERT new 'assistant' message avec actions proposées

    %% Le Frontend reçoit la réponse de l'IA en temps réel %%
    ChatDB-->>Frontend: Affiche la réponse IA et les boutons d'action

    User->>Frontend: Clique sur "Créer Note"
    Frontend->>API: POST /api/actions/execute { action: 'create_note', context_message_id }

    API->>EventStore: INSERT event { type: 'note.creation_requested', ... }

    %% Le travail de fond commence, l'UI peut afficher un spinner %%
    EventStore-->>Inngest: Déclenche le job 'onNoteCreationRequested'

    Inngest->>StateDB: INSERT dans 'entities' et 'content_blocks'
    Inngest->>EventStore: INSERT event { type: 'note.created', data: { entityId } }

    %% Le Frontend est notifié de la fin %%
    EventStore-->>Frontend: (Realtime) La note a été créée !
```
