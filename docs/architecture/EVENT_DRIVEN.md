...
│  ┌─────────────────────────────────────────────────────────┐  │
│  │ @synap/ai                                                │  │
│  │  • SynapAgent: LangGraph state machine (orchestration)  │  │
│  │  • LLM Calls: Vercel AI SDK (Anthropic Claude)          │  │
...
sendMessage: protectedProcedure
  ...
  .mutation(async ({ ctx, input }) => {
    ...
    const event = createSynapEvent({
      ...
      userId,
      aggregateId: input.threadId || randomUUID(),
      data: {
    ...
    // Publish to Inngest
    await inngest.send({ name: 'api/event.logged', data: { ... } });
    ...
    return {
      ...
      requestId,
      threadId: input.threadId,
      websocketUrl: `${REALTIME_URL}/rooms/user_${userId}/subscribe`,
    };
  })
```

**Note**: The actual AI processing happens asynchronously in `ConversationMessageHandler`, which:
1. Appends user message to conversation
2. Calls `runSynapAgent()` (LangGraph workflow)
3. Appends assistant response
4. Broadcasts result via WebSocket
...
#### Synap Agent (LangGraph State Machine)

The AI agent uses **LangGraph** for orchestration and **Vercel AI SDK** for LLM calls:

```typescript
// Workflow: parse_intent → gather_context → plan_actions → execute_actions → generate_final_response
export async function runSynapAgent(input: SynapAgentInput): Promise<SynapAgentResult> {
  return await synapAgentGraph.invoke({
    userId: input.userId,
    threadId: input.threadId,
...
**State Machine Nodes**:
1. **parse_intent** - Classifies user intent (capture/command/query) using Vercel AI SDK
2. **gather_context** - Semantic search + memory facts
3. **plan_actions** - Plans tool execution sequence using Vercel AI SDK
4. **execute_actions** - Executes tools (createEntity, semanticSearch, saveFact)
5. **generate_final_response** - Generates natural language response using Vercel AI SDK
...
const result = await generateObject({
  ...
  schema: classificationSchema, // Zod schema for type-safe output
  prompt: `${systemPrompt}\n\n${userMessage}`,
  temperature: 0,
});
```
...
**Benefits**:
- ✅ **LangGraph**: Perfect for multi-step reasoning workflows
- ✅ **Vercel AI SDK**: Simpler API, better TypeScript, built-in schema validation
- ✅ **Type-safe**: Zod schemas ensure structured outputs
- ✅ **Provider-agnostic**: Easy to switch AI providers via config
...
export function getEmbeddingsClient(): BaseEmbeddings {
  // Returns OpenAIEmbeddings or DeterministicEmbeddings
}
```
...
2. API Layer
   ↓
   • Publishes 'conversation.message.sent' event
   • Returns: { status: 'pending', requestId, websocketUrl }

3. ConversationMessageHandler (Inngest Worker)
   ↓
   • Appends user message to conversation (hash-chained)
   • Calls runSynapAgent() with LangGraph workflow

4. Synap Agent (LangGraph State Machine)
   ↓
   • parse_intent: Classifies intent (Vercel AI SDK)
   • gather_context: Semantic search + memory facts
   • plan_actions: Plans tool sequence (Vercel AI SDK)
   • execute_actions: Executes tools (createEntity, etc.)
   • generate_final_response: Generates response (Vercel AI SDK)

5. AI Response
   ↓
   • Text: "I'll create that task for you."
   • Actions executed: Task created via createEntity tool
   • Response stored in conversation (hash-chained)

6. Real-time Notification
   ↓
   • Handler broadcasts 'conversation.response.generated' via WebSocket
   • Client receives response in real-time

7. User Confirms (if needed)
   ↓
